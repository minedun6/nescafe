...
class PermissionDependencyTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $permission3Id = DB::table('permissions')->where('name', 'view-frontend')->first()->id;
        ...
        $visitsPermissionsIds = DB::table('permissions')->where('id', '>', 3)
            ->where('id', '<', 7)
            ->pluck('id');

        foreach ($visitsPermissionsIds  as $visitsPermissionId) {
            DB::table(config('access.permission_dependencies_table'))->insert([
                'permission_id' => $visitsPermissionId,
                'dependency_id' => $permission3Id,
                'created_at'    => Carbon::now(),
                'updated_at'    => Carbon::now(),
            ]);
        }
        $remainingPermissionsIds = DB::table('permissions')->where('id', '>', 6)->pluck('id');
